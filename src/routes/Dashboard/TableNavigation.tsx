...
const TableNavigation = ({ paging }: { paging: Paging | null }) => {
  ...
  return (
    <div className="relative w-full">
      <div className="flex w-full items-end justify-between gap-6 overflow-x-auto">
        <form onSubmit={handleSubmit}>
          <div className="flex gap-6">
            <div className="flex w-14 select-none flex-wrap gap-2 overflow-y-visible">
              <p className="font-display text-xs font-light">Limit</p>
              <span
                className="flex cursor-pointer rounded-sm border border-neutral-300 bg-neutral-100 px-3 py-2"
                onClick={() => {
                  if (internalSize) {
            ...
            <div className="flex select-none items-end overflow-y-visible">
              <div className="flex w-20 flex-wrap gap-2">
                <p className="font-display text-xs font-light">Jump to page</p>
                <span
                  className="flex w-full cursor-pointer rounded-sm border border-neutral-300 bg-neutral-100 px-3 py-2"
                  onClick={() => {
                    if (internalPage) {
      ...
      {pageDropdown && (
        <div className="absolute left-20 top-16 max-h-28 w-20 overflow-auto rounded-sm border border-neutral-300 bg-neutral-100 transition-all duration-300 ease-in-out">
